...
def create_args_parser():
    ...
    parser.add_argument(
        '-b', '--backup', '--b', dest='backup',
        help='Create a backup file (use --backup-dir for location)')
    parser.add_argument(
...
SCHEME_KEYWORDS = \
    ['define', 'local-odd?', 'when', 'begin', 'case',
     ...
     'with-implicit',
     'with-error-handler', 'module', 'parameterize', 'define-type',
     'define-mask', 'define-enum', 'module+',
     'match',
     'for', 'for/list', 'for/hash', 'for/vector']
...
def _after_indentation(res, options=None, fpath=''):
    ...
    if indented_code == res['original_code'] and options.files:
...
def indent_files(arguments):
    ...
    if not opts.files:
        ...
        _after_indentation(indent_result, options=opts)
    ...
    for fname in opts.files:
        ...
        _after_indentation(indent_result, options=opts, fpath=fname)
